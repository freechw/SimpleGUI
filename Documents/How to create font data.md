...
```
(6*2)*96+(12*2)*3755+(12*2)*3008 = 1152 + 90120 + 72192 = 163464 Bytes
```
。  
&emsp;&emsp;约164KB的Flash消耗对于单片机来说还是非常可观的，如果还需要其他不同大小的字体，空间资源需求还要成倍增加，所以这种时候，通常就要使用外部字库了。  
...
```
:1234567890
一二三四五六七八九十廿卅年月日时分秒立春雨水惊蛰清明谷夏小满芒种至暑大秋处白露寒霜降冬雪
```
&emsp;&emsp;可以看到，删除掉重复文字后，内容明显少了很多，也就是说我们只需要保存上述文字的字模即可，然后通过重新编写字符索引算法，将文字编码与字符索引联系起来即可。通常情况下，可以将这些被提取出来的字符在指定的编码下排序，然后通过二分法查找即可快速确定字符索引。  
&emsp;&emsp;此处以使用Unicode编码为例，讲解针对上述文字的精简方法。假定源码文件使用UTF-8编码，那么，首先需要编写的是一个将UTF-8编码还原成Unicode编码的函数，举例如下：
```c++
const char* GetUnicode(const char* cszHeader, uint32_t* puiUnicode)
{
    /*----------------------------------*/
	/* Variable Declaration				*/
	/*----------------------------------*/
    uint32_t     uiUnicode;
    uint16_t     uiUTF8Length = 0;
    uint16_t     uiSubsequent = 0;
    uint8_t      uiUTF8HeadByte = (uint8_t)(*cszHeader);
...
```
&emsp;&emsp;由于UTF-8编码是变长编码，每个字符占用1-6字节不等。通过以上的操作，UTF-8编码的字符会被还原成Unicode码，同时函数返回下一个字符的头字节指针。    
&emsp;&emsp;接下来，按照Unicode编码从小到大的顺序，将之前提取出来的文字排序，得到以下数据。    
```
0123456789:一七三九二五八六冬分十卅四处夏大寒小年廿惊日时明春暑月水清满白秋种秒立至芒蛰谷降雨雪霜露
```
&emsp;&emsp;这些文字的Unicode编码如下：  
```
0x0030, // U+0030  0
0x0031, // U+0031  1
0x0032, // U+0032  2
...
0x9732, // U+9732  露
```
&emsp;&emsp;这样我们就可以使用字符的Unicode编码值在这个被精简的字集中检索，并获取到在这个字集中的索引，进而可以得到描述字形的字模了。由于这个集合中Unicode编码是经过排序的，所以可以使用二分法等高效的查找算法以提升索引速度。    
&emsp;&emsp;至此，经过统计字库大小由143文字精简至99文字（在等宽字体中全角字符一个算两文字），实现了预期的精简字库的效果。  
